...
function [f, df] = CG_MNISTMod(VV,Dim,XX, l2Penalty)
...
Ix8=IO; 
dw8 =  w7probs'*Ix8 + [w8(1:end-1,:);zeros(1,size(w8,2))]*l2Penalty;

Ix7 = (Ix8*w8').*w7probs.*(1-w7probs); 
Ix7 = Ix7(:,1:end-1);
dw7 =  w6probs'*Ix7 + [w7(1:end-1,:);zeros(1,size(w7,2))]*l2Penalty;

Ix6 = (Ix7*w7').*w6probs.*(1-w6probs); 
Ix6 = Ix6(:,1:end-1);
dw6 =  w5probs'*Ix6 + [w6(1:end-1,:);zeros(1,size(w6,2))]*l2Penalty;

Ix5 = (Ix6*w6').*w5probs.*(1-w5probs); 
Ix5 = Ix5(:,1:end-1);
dw5 =  w4probs'*Ix5 + [w5(1:end-1,:);zeros(1,size(w5,2))]*l2Penalty;

Ix4 = (Ix5*w5');%.*w4probs.*(1-w4probs);
Ix4 = Ix4(:,1:end-1);
dw4 =  w3probs'*Ix4 + [w4(1:end-1,:);zeros(1,size(w4,2))]*l2Penalty;

Ix3 = (Ix4*w4').*w3probs.*(1-w3probs); 
Ix3 = Ix3(:,1:end-1);
dw3 =  w2probs'*Ix3 + [w3(1:end-1,:);zeros(1,size(w3,2))]*l2Penalty;

Ix2 = (Ix3*w3').*w2probs.*(1-w2probs); 
Ix2 = Ix2(:,1:end-1);
dw2 =  w1probs'*Ix2 + [w2(1:end-1,:);zeros(1,size(w2,2))]*l2Penalty;

Ix1 = (Ix2*w2').*w1probs.*(1-w1probs); 
Ix1 = Ix1(:,1:end-1);
dw1 =  XX'*Ix1 + [w1(1:end-1,:);zeros(1,size(w1,2))]*l2Penalty;
